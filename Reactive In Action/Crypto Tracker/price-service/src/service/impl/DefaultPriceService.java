...
public class DefaultPriceService implements PriceService {
	...
	Flux<MessageDTO<Float>> averagePrice(Flux<Long> requestedInterval,
		...
		return requestedInterval.defaultIfEmpty(DEFAULT_AVG_PRICE_INTERVAL)
				...
				.flatMap(messages -> messages.groupBy(MessageDTO::getCurrency))
				.flatMap(group ->
						group.reduce(Sum.empty(), (sum, next) -> sum.add(next.getData()))
								.map(Sum::avg)
								.map(avg -> MessageDTO.avg(avg, group.key(), "LocalMarketAvg")));
	}
}
