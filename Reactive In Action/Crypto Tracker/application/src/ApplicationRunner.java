...
public class ApplicationRunner {
	...
	public static Flux<Long> handleRequestedAveragePriceIntervalValue(Flux<String> requestedInterval) {
		...
		return requestedInterval
				.map(Long::parseLong)
				.filter(interval -> interval > 0 && interval <= 60)
				.onErrorContinue((error, interval) -> Mono.empty());
	}
	...
	public static Flux<String> handleOutgoingStreamBackpressure(Flux<String> outgoingStream) {
		...
		return outgoingStream.onBackpressureBuffer(Integer.MAX_VALUE, BufferOverflowStrategy.ERROR); // TODO Enable Backpressure
	}
